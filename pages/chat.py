...
def enhanced_qa_run(context, user_question):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if is_similar_to_bad_feedback(user_question, response):
            st.warning("⚠️ Detected response similar to previously flagged bad feedback. Retrying...")
        else:
            return response

    st.error("⚠️ Couldn't generate a completely new response after several tries. Returning last attempt.")
    return response
